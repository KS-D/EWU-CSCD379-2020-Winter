<template>
    <div>
        ...
        <gifts-details-component v-if="selectedGift != null"
                                  :gift="selectedGift"
                                  @gift-saved="refreshGifts()"></gifts-details-component>
    </div>
</template>
<script lang="ts">
    import {Vue, Component } from 'vue-property-decorator'
    import {Gift, GiftClient } from '../../secretsanta-client' 
    import GiftsDetailsComponent from './giftsDetailsComponent.vue'
    @Component({
        components: {
            GiftsDetailsComponent
        }
    })
    export default class GiftsComponent extends Vue {
        ...
        async deleteGift(gift : Gift){
            let giftClient = new GiftClient();
            if (confirm(`Are you sure you want to delete ${gift.title} ${gift.description}`)) {
